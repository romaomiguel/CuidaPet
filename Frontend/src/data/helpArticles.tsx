...
export const HELP_ARTICLES: HelpArticle[] = [
  // ─── Sou Cliente ──────────────────────────────────────────────────────
  {
    ...
    content: `Encontrar e reservar um cuidador no CuidaPet é rápido. Veja o passo a passo:

Comece pelo match inteligente: responda algumas perguntas rápidas sobre o seu pet, o local, a data e o valor que pretende investir. Com base nisso, o CuidaPet calcula e apresenta uma lista de cuidadores ordenada por compatibilidade, mostrando a porcentagem de match de cada um.
...
A partir daí, vocês podem combinar os detalhes pelo chat, e você acompanha tudo pela sua área de agendamentos.`,
  ...
  },
  {
    ...
    content: `Sabemos que imprevistos acontecem. Veja como funcionam os cancelamentos no CuidaPet:
...
Mantenha os dados sempre atualizados, especialmente antes de um novo agendamento, para que o cuidador tenha todas as informações necessárias.`,
  ...
  // ─── A Plataforma ─────────────────────────────────────────────────────
  {
    ...
    content: `Sua segurança é importante para nós. Siga essas orientações para uma experiência tranquila no CuidaPet:

**Nunca compartilhe sua senha**
A equipe do CuidaPet nunca irá pedir sua senha por chat, e-mail ou telefone. Se alguém pedir, não forneça e denuncie pela Central de Ajuda.

**Faça pagamentos apenas pela plataforma**
Isso garante que você está protegido pelas políticas do CuidaPet. Combinações de pagamento fora da plataforma não têm nenhuma garantia ou suporte da nossa parte.
...
Manter a conversa dentro do CuidaPet cria um histórico que pode ser útil em caso de qualquer divergência sobre o serviço combinado.
...
Esta Política de Privacidade explica como o CuidaPet coleta, usa, armazena e protege os dados pessoais dos usuários, em conformidade com a Lei Geral de Proteção de Dados (LGPD).
...
Esta política pode ser atualizada para refletir mudanças na plataforma ou na legislação. Alterações relevantes serão comunicadas.`,
  ...
  },
  {
    ...
    slug: 'termos-de-uso',
    title: 'Termos de Uso',
    excerpt: 'Regras de uso da plataforma CuidaPet.',
    content: `Última atualização: julho de 2026

Bem-vindo ao CuidaPet. Estes Termos de Uso regulam o acesso e uso da plataforma, que conecta tutores de animais de estimação a cuidadores (petsitters) para prestação de serviços de cuidado com pets. Ao criar uma conta, você concorda com os termos abaixo.

**1. Cadastro**
Para usar o CuidaPet, é necessário criar uma conta como tutor ou como petsitter, fornecendo informações verdadeiras e atualizadas. Você é responsável por manter a confidencialidade da sua senha e por todas as atividades realizadas em sua conta.

**2. O papel do CuidaPet**
O CuidaPet é uma plataforma de intermediação: conecta tutores e cuidadores, mas não presta os serviços de cuidado diretamente. Os agendamentos, combinações e a execução do serviço são de responsabilidade exclusiva das partes envolvidas — tutor e petsitter.

**3. Cadastro de petsitters**
Os Petsitters devem ser maiores de 18 anos e devem ser solicitados a enviar documentos de identificação para fins de verificação. O CuidaPet realiza uma análise razoável, mas não garante nem se responsabiliza integralmente pela conduta dos petsitters cadastrados. Tutores devem exercer seu próprio julgamento ao escolher um petsitter.
...
**5. Avaliações**
Usuários podem avaliar os serviços prestados após sua conclusão. Avaliações devem refletir experiências reais e não devem conter conteúdo ofensivo, discriminatório ou falso. O CuidaPet pode remover avaliações que violem esta regra.
...
**7. Pagamentos**
O CuidaPet utilizará uma plataforma de pagamento parceira para processar todos os pagamentos realizados pelos tutores. Os valores devidos aos petsitters serão repassados pela plataforma parceira, descontando-se a comissão devida ao CuidaPet, conforme definida em Políticas de Pagamento específicas.

O tutor pagará o valor total do serviço diretamente ao CuidaPet no momento da reserva, que reterá esses valores até a data de pagamento dos petsitters, conforme as políticas da plataforma parceira.

**8. Limitação de responsabilidade**
O CuidaPet não se responsabiliza por danos, prejuízos ou incidentes ocorridos durante a prestação dos serviços entre tutor e petsitter, atuando como intermediário da conexão entre as partes.
...
Dúvidas sobre estes termos podem ser enviadas pela Central de Ajuda.`,
  ...
  // ─── Sou Petsitter — Começar na plataforma ────────────────────────────
  {
    ...
    content: `Bem-vindo ao CuidaPet. Tornar-se um cuidador na plataforma é o primeiro passo para receber agendamentos, e tudo começa por um perfil bem montado — é ele que os tutores veem quando o CuidaPet recomenda você.
...
Quanto mais completo e verdadeiro for o seu perfil, mais confiança ele transmite — e confiança é o que faz um tutor escolher você entre os cuidadores recomendados.`,
  ...
  },
  {
    ...
    content: `Para manter o CuidaPet um ambiente seguro para todos, pedimos que os cuidadores enviem documentos de verificação. Isso protege os tutores e valoriza você, que demonstra ser um profissional sério e confiável.

Na sua área de perfil, você encontra o espaço para enviar seus documentos, como comprovante de identidade e comprovante de endereço. O envio é feito de forma segura: seus documentos ficam armazenados de maneira protegida e são acessíveis apenas à equipe do CuidaPet para a verificação — nenhum tutor tem acesso a eles, após a verificação os documentos ficam na base de dados por 7 dias e após isso são excluídos.
...
Envie arquivos legíveis e dentro do prazo de validade. Depois de enviados, seus documentos passam por uma análise da plataforma. Manter essa etapa em dia deixa seu perfil mais confiável e pronto para receber agendamentos.`,
  ...
  // ─── Sou Petsitter — Aparecer e ser escolhido ─────────────────────────
  {
    ...
    content: `No CuidaPet, o tutor não fica procurando cuidadores um por um. Ele responde a algumas perguntas — sobre o animal, o local, a data e o valor — e o nosso match inteligente apresenta a ele uma lista de cuidadores ordenada por compatibilidade, com uma porcentagem de match para cada um.

Ou seja: em vez de você disputar atenção numa lista enorme, o CuidaPet trabalha para colocar você na frente dos tutores certos — aqueles cuja necessidade combina com o que você oferece. Quanto melhor o seu encaixe com o que o tutor procura, mais alto você aparece.
...
Alguns cuidados ajudam a melhorar a sua posição e a chance de ser escolhido: mantenha seus serviços e disponibilidade atualizados, defina preços coerentes com o que você oferece, complete seu perfil e cuide da sua reputação — avaliações boas contam muito na hora de o tutor decidir. Recursos como o compartilhamento de localização durante os passeios também são um diferencial que os tutores valorizam.`,
  ...
  },
  {
    ...
    content: `Seus serviços e preços definem o que você oferece e quanto cobra — e são usados pelo CuidaPet tanto para apresentar você aos tutores quanto para calcular o valor de cada agendamento.
...
Durante um passeio, dentro do agendamento ativo, você encontra o botão para compartilhar a sua localização naquele momento. A cada vez que você aperta, o CuidaPet registra o ponto onde você está, e o tutor consegue ver esse trajeto no mapa. Você decide quando e quantas vezes compartilhar — pode marcar a saída, a chegada ao parque, o caminho de volta.
...
Vale saber: cada ponto que você compartilha fica registrado no histórico do serviço, visível para o tutor, mesmo depois que o passeio termina. Compartilhe à vontade nos momentos do passeio — é uma forma simples de mostrar cuidado e transparência.`,
  ...
  // ─── Sou Petsitter — Reputação e ganhos ───────────────────────────────
  {
    ...
    content: `Depois de cada serviço concluído, o tutor pode avaliar o seu trabalho com uma nota, etiquetas rápidas e um comentário. Essas avaliações aparecem no seu perfil e formam a sua reputação no CuidaPet.
