...
import { Menu, X, ChevronDown, Search, LogOut, User, CalendarDays, PawPrint, LayoutDashboard } from 'lucide-react'
...
export function PublicLayout() {
  ...
  const adminLinks = [
    { to: '/admin',            icon: <LayoutDashboard size={15} />, label: 'Painel Admin' },
    { to: '/admin/users',      icon: <User size={15} />,            label: 'Usuários'     },
    { to: '/admin/petsitters', icon: <PawPrint size={15} />,        label: 'Petsitters'   },
  ]
        ...
        {mobileOpen && (
          <div className="md:hidden bg-white border-t border-stroke shadow-card animate-slide-up">
            <div className="px-4 py-3 space-y-1">
              ...
              {isAuthenticated && accountLinks.map(({ to, icon, label }) => (
